...
public class Book {
	...
	public static List<Book> fromString(String booksStr) {
        List<Book> books = new ArrayList<>();
        if (booksStr == null || booksStr.trim().isEmpty()) {
            return books;
        }
        
        // Assuming the books string is in format: "title1,author1;title2,author2"
        String[] bookEntries = booksStr.split(";");
        for (String bookEntry : bookEntries) {
            String[] parts = bookEntry.split(",");
            if (parts.length >= 2) {
                Book book = new Book();
                book.setTitle(parts[0].trim());
                book.setPublisher_name(parts[1].trim());
                books.add(book);
            }
        }
        return books;
